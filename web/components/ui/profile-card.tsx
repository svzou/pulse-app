...
import { WorkoutAuthor } from "@/utils/supabase/models/workout";
...
export default function UserProfile() {
  ...
  const { data: userData, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      return { user: data.user, profile: WorkoutAuthor.parse(profileData) };
    },
  });
  ...
  return (
    <div className="flex items-center mt-8 gap-2">
      ...
      <Avatar className="h-10 w-10">
        ...
        />
        <AvatarFallback>
          {profile.full_name ? profile.full_name.slice(0, 2).toUpperCase() : "UN"}
        </AvatarFallback>
      </Avatar>
    </div>
  );
